package com.travelbenefits.app.data.catalog

import com.travelbenefits.app.domain.model.AwardPriceBand
import com.travelbenefits.app.domain.model.BalanceUnit
import com.travelbenefits.app.domain.model.ExpirationPolicy
import com.travelbenefits.app.domain.model.LoyaltyProgram
import com.travelbenefits.app.domain.model.LoyaltyProgramKind
import com.travelbenefits.app.domain.model.ProgramProfile
import com.travelbenefits.app.domain.model.QualifyingMetric
import com.travelbenefits.app.domain.model.TierLevel

/**
 * Hand-curated profiles for every [LoyaltyProgram]: elite ladders,
 * expiration rules, earning rates, rough award prices and links.
 *
 * ACCURACY NOTE: programs revise these rules every year or two (several did
 * in 2024-2026). Everything here is "as understood on [ProgramProfile.dataAsOf]"
 * and is surfaced in the UI with that caveat. Where a threshold couldn't be
 * pinned down with confidence it is left null rather than guessed, so the
 * app shows the tier name without a progress bar instead of a wrong number.
 */
object LoyaltyProgramCatalog {

    private const val AS_OF = "September 2026 snapshot - verify with the program before relying on it"

    val profiles: Map<LoyaltyProgram, ProgramProfile> = listOf(
        ProgramProfile(
            program = LoyaltyProgram.MARRIOTT_BONVOY,
            awardStaysCountForStatus = true,
            qualificationYear = "calendar year",
            poolingRule = "Members can transfer up to 100,000 points per year to another member (both accounts 30+ days old); no shared pool.",
            qualifyingMetric = QualifyingMetric.NIGHTS,
            tiers = listOf(
                TierLevel("Silver", 10, perks = "10% bonus points, late checkout when available"),
                TierLevel("Gold", 25, perks = "25% bonus points, 2pm late checkout, room upgrades when available"),
                TierLevel("Platinum", 50, perks = "50% bonus points, lounge access, 4pm checkout, welcome gift choice"),
                TierLevel("Titanium", 75, perks = "75% bonus points, plus Platinum perks and annual choice benefit"),
                TierLevel("Ambassador", 100, altQualification = "plus \$23,000 annual qualifying spend", perks = "Ambassador service, Your24 check-in"),
            ),
            expiration = ExpirationPolicy(24, "Points expire after 24 months with no qualifying activity (earning or redeeming resets the clock)."),
            estValueCentsPerPoint = 0.8,
            basePointsPerDollar = 10.0,
            eliteBonusByTier = mapOf("Silver" to 0.10, "Gold" to 0.25, "Platinum" to 0.50, "Titanium" to 0.75, "Ambassador" to 0.75),
            awardBand = AwardPriceBand(10_000, 35_000, 100_000, "Dynamic pricing; budget brands from ~10k, luxury/peak 100k+."),
            accountUrl = "https://www.marriott.com/loyalty/myAccount/default.mi",
            awardSearchUrl = "https://www.marriott.com/search/default.mi?useRewardsPoints=true",
            brandKeywords = listOf("marriott", "bonvoy", "sheraton", "westin", "courtyard", "ritz-carlton", "ritz carlton", "st. regis", "st regis", "w hotel", "residence inn", "fairfield", "springhill", "aloft", "moxy", "le meridien", "le méridien", "jw marriott", "autograph collection", "tribute portfolio", "renaissance", "four points", "element", "delta hotels", "gaylord", "edition", "luxury collection", "towneplace", "ac hotel", "protea", "design hotels", "city express"),
            dataAsOf = AS_OF,
        ),
        ProgramProfile(
            program = LoyaltyProgram.HILTON_HONORS,
            awardStaysCountForStatus = true,
            qualificationYear = "calendar year",
            poolingRule = "Points Pooling lets up to 11 members combine points free of charge.",
            qualifyingMetric = QualifyingMetric.NIGHTS,
            tiers = listOf(
                TierLevel("Silver", 10, altQualification = "or 4 stays", perks = "20% bonus points, 5th standard reward night free"),
                TierLevel("Gold", 40, altQualification = "or 20 stays", perks = "80% bonus points, daily F&B credit (US) or breakfast, space-available upgrades"),
                TierLevel("Diamond", 60, altQualification = "or 30 stays", perks = "100% bonus points, executive lounge access, premium wifi"),
            ),
            expiration = ExpirationPolicy(24, "Points expire after 24 months of inactivity; any earn or redeem resets the clock."),
            estValueCentsPerPoint = 0.5,
            basePointsPerDollar = 10.0,
            eliteBonusByTier = mapOf("Silver" to 0.20, "Gold" to 0.80, "Diamond" to 1.00),
            awardBand = AwardPriceBand(10_000, 50_000, 150_000, "Dynamic; 5th night free on standard awards for elites."),
            accountUrl = "https://www.hilton.com/en/hilton-honors/",
            awardSearchUrl = "https://www.hilton.com/en/book/reservation/rooms/?redeemPts=true",
            brandKeywords = listOf("hilton", "honors", "hampton", "doubletree", "embassy suites", "homewood", "home2", "tru by hilton", "waldorf", "conrad", "canopy", "curio", "tapestry", "lxr", "signia", "motto", "tempo", "spark by hilton", "graduate hotels", "nomad"),
            dataAsOf = AS_OF,
        ),
        ProgramProfile(
            program = LoyaltyProgram.WORLD_OF_HYATT,
            awardStaysCountForStatus = true,
            qualificationYear = "calendar year",
            poolingRule = "Points can be transferred to another member with a request form; no pool.",
            qualifyingMetric = QualifyingMetric.NIGHTS,
            tiers = listOf(
                TierLevel("Discoverist", 10, altQualification = "or 25,000 base points", perks = "10% bonus points, 2pm checkout, premium wifi"),
                TierLevel("Explorist", 30, altQualification = "or 50,000 base points", perks = "20% bonus points, room upgrades, 4 Club lounge awards"),
                TierLevel("Globalist", 60, altQualification = "or 100,000 base points", perks = "30% bonus points, club access, free breakfast, suite upgrades, waived resort fees"),
            ),
            expiration = ExpirationPolicy(24, "Points expire after 24 months without earning or redeeming activity."),
            estValueCentsPerPoint = 1.7,
            basePointsPerDollar = 5.0,
            eliteBonusByTier = mapOf("Discoverist" to 0.10, "Explorist" to 0.20, "Globalist" to 0.30),
            awardBand = AwardPriceBand(3_500, 15_000, 45_000, "Category-based chart with off-peak/standard/peak pricing."),
            accountUrl = "https://world.hyatt.com/content/gp/en/account.html",
            awardSearchUrl = "https://www.hyatt.com/search/hotels/?rate=Standard&rateFilter=woh",
            brandKeywords = listOf("hyatt", "andaz", "grand hyatt", "park hyatt", "hyatt regency", "hyatt place", "hyatt house", "thompson", "alila", "miraval", "caption by hyatt", "destination by hyatt", "jdv", "unbound collection", "hyatt centric", "hyatt ziva", "hyatt zilara", "dream hotels", "mr & mrs smith"),
            dataAsOf = AS_OF,
        ),
        ProgramProfile(
            program = LoyaltyProgram.IHG_ONE_REWARDS,
            awardStaysCountForStatus = true,
            qualificationYear = "calendar year",
            poolingRule = "Points can be transferred between members for a fee; verify current terms.",
            qualifyingMetric = QualifyingMetric.NIGHTS,
            tiers = listOf(
                TierLevel("Silver", 10, perks = "20% bonus points"),
                TierLevel("Gold", 20, perks = "40% bonus points, welcome points or amenity"),
                TierLevel("Platinum", 40, perks = "60% bonus points, space-available upgrades, guaranteed 2pm checkout"),
                TierLevel("Diamond", 70, perks = "100% bonus points, complimentary breakfast at most brands, lounge access"),
            ),
            expiration = ExpirationPolicy(12, "Points expire after 12 months of inactivity for members without elite status; elite members' points don't expire while status is held."),
            estValueCentsPerPoint = 0.5,
            basePointsPerDollar = 10.0,
            eliteBonusByTier = mapOf("Silver" to 0.20, "Gold" to 0.40, "Platinum" to 0.60, "Diamond" to 1.00),
            awardBand = AwardPriceBand(10_000, 35_000, 100_000, "Dynamic; 4th night free on 4+ night awards for IHG credit cardholders."),
            accountUrl = "https://www.ihg.com/onerewards/content/us/en/home",
            awardSearchUrl = "https://www.ihg.com/hotels/us/en/reservation?rateCode=IVANI",
            brandKeywords = listOf("ihg", "intercontinental", "holiday inn", "crowne plaza", "kimpton", "hotel indigo", "staybridge", "candlewood", "even hotels", "avid", "voco", "six senses", "regent", "vignette", "atwell", "garner", "hualuxe"),
            dataAsOf = AS_OF,
        ),
        ProgramProfile(
            program = LoyaltyProgram.WYNDHAM_REWARDS,
            qualifyingMetric = QualifyingMetric.NIGHTS,
            tiers = listOf(
                TierLevel("Gold", 7, perks = "Late checkout, preferred rooms"),
                TierLevel("Platinum", 16, perks = "10% bonus points, early check-in"),
                TierLevel("Diamond", 40, perks = "15% bonus points, suite upgrades, welcome amenity"),
            ),
            expiration = ExpirationPolicy(18, "Points expire after 18 months of inactivity, and in any case 4 years after they were earned."),
            estValueCentsPerPoint = 1.0,
            basePointsPerDollar = 10.0,
            eliteBonusByTier = mapOf("Platinum" to 0.10, "Diamond" to 0.15),
            awardBand = AwardPriceBand(7_500, 15_000, 30_000, "Fixed tiers of 7,500 / 15,000 / 30,000 points per night (Vacasa/Vacation rentals differ)."),
            accountUrl = "https://www.wyndhamhotels.com/wyndham-rewards/account",
            awardSearchUrl = "https://www.wyndhamhotels.com/wyndham-rewards/redeem",
            brandKeywords = listOf("wyndham", "la quinta", "ramada", "days inn", "super 8", "microtel", "wingate", "baymont", "travelodge", "howard johnson", "hawthorn", "americinn", "trademark", "dolce", "registry collection", "vienna house", "echo suites"),
            dataAsOf = AS_OF,
        ),
        ProgramProfile(
            program = LoyaltyProgram.CHOICE_PRIVILEGES,
            qualifyingMetric = QualifyingMetric.NIGHTS,
            tiers = listOf(
                TierLevel("Gold", 10, perks = "10% bonus points"),
                TierLevel("Platinum", 20, perks = "25% bonus points, dedicated line"),
                TierLevel("Diamond", 40, perks = "50% bonus points, room upgrades, welcome gift"),
            ),
            expiration = ExpirationPolicy(18, "Points expire after 18 months without qualifying activity."),
            estValueCentsPerPoint = 0.6,
            basePointsPerDollar = 10.0,
            eliteBonusByTier = mapOf("Gold" to 0.10, "Platinum" to 0.25, "Diamond" to 0.50),
            awardBand = AwardPriceBand(8_000, 16_000, 35_000, "Mostly 8k-35k per night in the US; Europe/upscale brands higher."),
            accountUrl = "https://www.choicehotels.com/choice-privileges",
            awardSearchUrl = "https://www.choicehotels.com/choice-privileges/redeem",
            brandKeywords = listOf("choice hotels", "choice privileges", "comfort inn", "comfort suites", "quality inn", "sleep inn", "clarion", "cambria", "ascend", "mainstay", "suburban", "woodspring", "econo lodge", "rodeway", "radisson blu", "radisson red", "park inn", "country inn"),
            dataAsOf = AS_OF,
            notes = "Radisson Hotels Americas was folded into Choice Privileges in 2022-23, so US/Canada Radisson stays typically credit here rather than to Radisson Rewards.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.ACCOR_LIVE_LIMITLESS,
            qualifyingMetric = QualifyingMetric.STATUS_POINTS,
            tiers = listOf(
                TierLevel("Silver", 2_000, altQualification = "or 10 nights", perks = "Welcome drink, late checkout"),
                TierLevel("Gold", 7_000, altQualification = "or 30 nights", perks = "Room upgrade, welcome drink, early check-in"),
                TierLevel("Platinum", 14_000, altQualification = "or 60 nights", perks = "Lounge access, suite upgrades where available"),
                TierLevel("Diamond", 26_000, perks = "Free breakfast on weekends, dining perks"),
            ),
            expiration = ExpirationPolicy(12, "Reward points expire after 12 months without earning activity."),
            estValueCentsPerPoint = 2.2,
            basePointsPerDollar = 2.5,
            eliteBonusByTier = mapOf("Silver" to 0.25, "Gold" to 0.50, "Platinum" to 0.75, "Diamond" to 1.00),
            awardBand = AwardPriceBand(2_000, 4_000, 10_000, "Fixed value: every 2,000 points = EUR 40 off a booking - no award chart."),
            accountUrl = "https://all.accor.com/",
            awardSearchUrl = "https://all.accor.com/",
            brandKeywords = listOf("accor", "sofitel", "novotel", "ibis", "mercure", "pullman", "fairmont", "raffles", "swissôtel", "swissotel", "mgallery", "mövenpick", "movenpick", "banyan tree", "rixos", "sls", "mondrian", "delano", "hyde", "the hoxton", "ennismore", "orient express"),
            dataAsOf = AS_OF,
            notes = "Earning is spend-based in euros (points per EUR 10), so basePointsPerDollar is a rough USD conversion.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.BEST_WESTERN_REWARDS,
            qualifyingMetric = QualifyingMetric.NIGHTS,
            tiers = listOf(
                TierLevel("Gold", 10, perks = "10% bonus points"),
                TierLevel("Platinum", 15, perks = "15% bonus points"),
                TierLevel("Diamond", 30, perks = "30% bonus points, room upgrades"),
                TierLevel("Diamond Select", 50, perks = "50% bonus points"),
            ),
            expiration = ExpirationPolicy(null, "Points don't expire."),
            estValueCentsPerPoint = 0.6,
            basePointsPerDollar = 10.0,
            eliteBonusByTier = mapOf("Gold" to 0.10, "Platinum" to 0.15, "Diamond" to 0.30, "Diamond Select" to 0.50),
            awardBand = AwardPriceBand(8_000, 20_000, 36_000, "Property-based pricing, roughly 8k-36k per night."),
            accountUrl = "https://www.bestwestern.com/en_US/rewards.html",
            awardSearchUrl = "https://www.bestwestern.com/en_US/rewards/redeem.html",
            brandKeywords = listOf("best western", "bw premier", "surestay", "glō", "glo hotel", "vīb", "aiden", "sadie", "executive residency", "worldhotels"),
            dataAsOf = AS_OF,
        ),
        ProgramProfile(
            program = LoyaltyProgram.RADISSON_REWARDS,
            qualifyingMetric = QualifyingMetric.NIGHTS,
            tiers = listOf(
                TierLevel("Premium", null),
                TierLevel("VIP", null),
            ),
            expiration = ExpirationPolicy(24, "Points expire after 24 months of inactivity (rule not re-verified recently)."),
            estValueCentsPerPoint = 0.3,
            basePointsPerDollar = 20.0,
            accountUrl = "https://www.radissonhotels.com/en-us/rewards",
            awardSearchUrl = "https://www.radissonhotels.com/en-us/rewards/redeem",
            brandKeywords = listOf("radisson collection", "radisson individuals", "art'otel", "prizeotel"),
            dataAsOf = AS_OF,
            notes = "Applies to Radisson Rewards outside the Americas only; US/Canada Radisson stays credit to Choice Privileges. Tier thresholds intentionally left blank - verify at radissonhotels.com.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.DELTA_SKYMILES,
            awardStaysCountForStatus = null,
            qualificationYear = "calendar year (Medallion Qualification Year)",
            poolingRule = "Miles can be transferred for a fee; no family pooling. Whether award tickets earn MQDs is not verified here.",
            qualifyingMetric = QualifyingMetric.QUALIFYING_DOLLARS,
            tiers = listOf(
                TierLevel("Silver Medallion", 5_000, perks = "Unlimited complimentary upgrades (lowest priority), free checked bag, priority boarding"),
                TierLevel("Gold Medallion", 10_000, perks = "Higher upgrade priority, Sky Priority, waived same-day change fees"),
                TierLevel("Platinum Medallion", 15_000, perks = "Choice Benefits, regional upgrade certificates"),
                TierLevel("Diamond Medallion", 28_000, perks = "Top priority, global upgrade certificates, Choice Benefits"),
            ),
            expiration = ExpirationPolicy(null, "SkyMiles never expire."),
            estValueCentsPerPoint = 1.2,
            basePointsPerDollar = 5.0,
            eliteBonusByTier = mapOf("Silver Medallion" to 0.40, "Gold Medallion" to 0.60, "Platinum Medallion" to 0.80, "Diamond Medallion" to 1.20),
            awardBand = AwardPriceBand(6000, 14000, 35000, "One-way domestic economy; fully dynamic, flash sales from ~6k, peak 35k+."),
            accountUrl = "https://www.delta.com/us/en/skymiles/overview",
            awardSearchUrl = "https://www.delta.com/flightsearch/book-a-flight?awardTravel=true",
            brandKeywords = listOf("delta air lines", "delta airlines", "delta.com", "skymiles", "delta flight"),
            dataAsOf = AS_OF,
            notes = "Status is Medallion Qualification Dollars (MQDs) only since 2024; the MQD thresholds above are the widely reported 2025 program-year figures.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.UNITED_MILEAGEPLUS,
            awardStaysCountForStatus = null,
            qualificationYear = "calendar year",
            poolingRule = "MileagePlus Pool lets up to 5 members share miles (some restrictions).",
            qualifyingMetric = QualifyingMetric.QUALIFYING_POINTS,
            tiers = listOf(
                TierLevel("Premier Silver", 6_000, altQualification = "or 5,000 PQP + 15 PQF", perks = "Complimentary Economy Plus at check-in, free checked bag"),
                TierLevel("Premier Gold", 12_000, altQualification = "or 10,000 PQP + 30 PQF", perks = "Star Alliance Gold, lounge access on international itineraries"),
                TierLevel("Premier Platinum", 18_000, altQualification = "or 15,000 PQP + 45 PQF", perks = "PlusPoints upgrades, higher upgrade priority"),
                TierLevel("Premier 1K", 28_000, altQualification = "or 22,000 PQP + 60 PQF", perks = "Top-tier upgrades and service"),
            ),
            expiration = ExpirationPolicy(null, "MileagePlus miles don't expire."),
            estValueCentsPerPoint = 1.3,
            basePointsPerDollar = 5.0,
            eliteBonusByTier = mapOf("Premier Silver" to 0.40, "Premier Gold" to 0.60, "Premier Platinum" to 0.80, "Premier 1K" to 1.20),
            awardBand = AwardPriceBand(6000, 12500, 30000, "One-way domestic economy; dynamic, saver-level from ~6k, peak 30k+."),
            accountUrl = "https://www.united.com/en/us/account",
            awardSearchUrl = "https://www.united.com/en/us/book-flight/united-reservations?awardTravel=true",
            brandKeywords = listOf("united airlines", "united.com", "mileageplus", "united flight"),
            dataAsOf = AS_OF,
            notes = "Thresholds are Premier Qualifying Points (PQP) for the PQP-only path; PQP+PQF combinations shown as the alternate. United raised these for 2026 - double-check united.com.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.SOUTHWEST_RAPID_REWARDS,
            awardStaysCountForStatus = null,
            qualificationYear = "calendar year",
            poolingRule = "Points can be transferred to other members for a fee.",
            qualifyingMetric = QualifyingMetric.QUALIFYING_POINTS,
            tiers = listOf(
                TierLevel("A-List", 35_000, altQualification = "or 20 qualifying one-way flights", perks = "Priority boarding, 25% earning bonus, same-day standby"),
                TierLevel("A-List Preferred", 70_000, altQualification = "or 40 qualifying one-way flights", perks = "100% earning bonus, free inflight wifi"),
            ),
            expiration = ExpirationPolicy(null, "Rapid Rewards points don't expire."),
            estValueCentsPerPoint = 1.3,
            basePointsPerDollar = 6.0,
            eliteBonusByTier = mapOf("A-List" to 0.25, "A-List Preferred" to 1.00),
            awardBand = AwardPriceBand(4000, 10000, 25000, "One-way; fixed-value pricing tied to the cash fare (~1.3-1.5¢/pt)."),
            accountUrl = "https://www.southwest.com/myaccount/",
            awardSearchUrl = "https://www.southwest.com/air/booking/?fareType=POINTS",
            brandKeywords = listOf("southwest airlines", "southwest.com", "rapid rewards", "southwest flight"),
            dataAsOf = AS_OF,
            notes = "Southwest overhauled fares, bags and boarding in 2025; earning rates per fare bucket changed - verify before counting on points.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.AMERICAN_AADVANTAGE,
            awardStaysCountForStatus = false,
            qualificationYear = "March 1 - end of February qualification year",
            poolingRule = "No family pooling; miles transfer for a fee. Award tickets do not earn Loyalty Points.",
            qualifyingMetric = QualifyingMetric.QUALIFYING_POINTS,
            tiers = listOf(
                TierLevel("Gold", 40_000, perks = "40% mileage bonus, free checked bag, complimentary upgrades"),
                TierLevel("Platinum", 75_000, perks = "60% bonus, oneworld Sapphire, lounge access on international itineraries"),
                TierLevel("Platinum Pro", 125_000, perks = "80% bonus, higher upgrade priority"),
                TierLevel("Executive Platinum", 200_000, perks = "120% bonus, systemwide upgrades, oneworld Emerald"),
            ),
            expiration = ExpirationPolicy(24, "Miles expire after 24 months of inactivity (members under 65); any earn or redeem resets the clock."),
            estValueCentsPerPoint = 1.4,
            basePointsPerDollar = 5.0,
            eliteBonusByTier = mapOf("Gold" to 0.40, "Platinum" to 0.60, "Platinum Pro" to 0.80, "Executive Platinum" to 1.20),
            awardBand = AwardPriceBand(7500, 15000, 30000, "One-way domestic economy; web specials from 7.5k, dynamic peak 30k+."),
            accountUrl = "https://www.aa.com/aadvantage-program/profile/account-summary",
            awardSearchUrl = "https://www.aa.com/booking/find-flights?redeemMiles=true",
            brandKeywords = listOf("american airlines", "aa.com", "aadvantage", "american flight"),
            dataAsOf = AS_OF,
            notes = "Status is measured in Loyalty Points (earned from flights, credit cards and partners) per qualification year.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.ATMOS_REWARDS_AIRLINE,
            qualifyingMetric = QualifyingMetric.STATUS_POINTS,
            tiers = listOf(
                TierLevel("Silver", null),
                TierLevel("Gold", null),
                TierLevel("Platinum", null),
                TierLevel("Titanium", null),
            ),
            expiration = ExpirationPolicy(null, "Miles don't expire on inactivity (post-2024 policy); verify at atmosrewards.com."),
            estValueCentsPerPoint = 1.5,
            basePointsPerDollar = null,
            awardBand = AwardPriceBand(5000, 12500, 30000, "One-way domestic economy on Alaska/Hawaiian; distance-based pricing from 5k."),
            accountUrl = "https://www.alaskaair.com/account/overview",
            awardSearchUrl = "https://www.alaskaair.com/planbook?awardType=MilesOnly",
            brandKeywords = listOf("alaska airlines", "alaskaair", "hawaiian airlines", "hawaiianairlines", "atmos rewards", "mileage plan", "hawaiianmiles"),
            dataAsOf = AS_OF,
            notes = "Atmos Rewards replaced Mileage Plan / HawaiianMiles in 2025 and its status-point thresholds are still settling, so they're intentionally left blank rather than guessed.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.JETBLUE_TRUEBLUE,
            awardStaysCountForStatus = null,
            qualificationYear = "calendar year",
            poolingRule = "Points Pooling lets family groups combine points.",
            qualifyingMetric = QualifyingMetric.TILES,
            tiers = listOf(
                TierLevel("Mosaic 1", 50, perks = "Free checked bags, early boarding, Even More Space at check-in"),
                TierLevel("Mosaic 2", 100, perks = "Adds a Mosaic perk choice"),
                TierLevel("Mosaic 3", 150, perks = "Adds Mint upgrade certificates"),
                TierLevel("Mosaic 4", 250, perks = "Top tier: more Mint certificates and perks"),
            ),
            expiration = ExpirationPolicy(null, "TrueBlue points don't expire."),
            estValueCentsPerPoint = 1.3,
            basePointsPerDollar = 3.0,
            awardBand = AwardPriceBand(5000, 12000, 30000, "One-way; fixed-value pricing tied to the cash fare (~1.3¢/pt)."),
            accountUrl = "https://trueblue.jetblue.com/",
            awardSearchUrl = "https://www.jetblue.com/booking/flights?usePoints=true",
            brandKeywords = listOf("jetblue", "trueblue"),
            dataAsOf = AS_OF,
            notes = "Roughly 1 tile per \$100 spent with JetBlue (flights, vacations, co-brand card spend).",
        ),

        // ---- Shops, dining and shopping rewards ------------------------------
        // Valuations here are "what a point is worth at the counter"; most of
        // these have no tiers and short expiry windows. Nothing transfers in
        // or out of them, so they only ever appear in the wallet and alerts.
        ProgramProfile(
            program = LoyaltyProgram.AMAZON,
            qualifyingMetric = QualifyingMetric.QUALIFYING_DOLLARS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(null, "Gift card balances don't expire; promotional credits usually carry their own dates."),
            estValueCentsPerPoint = 100.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.amazon.com/gc/balance",
            awardSearchUrl = "https://www.amazon.com/",
            brandKeywords = listOf("amazon.com", "amazon gift card"),
            dataAsOf = AS_OF,
            balanceUnit = BalanceUnit.DOLLARS,
            notes = "Tracks the gift-card/rewards balance in dollars (the metric Navan Edge's wallet shows).",
        ),
        ProgramProfile(
            program = LoyaltyProgram.STARBUCKS_REWARDS,
            qualifyingMetric = QualifyingMetric.STATUS_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(6, "Stars expire 6 months after the month they were earned."),
            estValueCentsPerPoint = 5.0,
            basePointsPerDollar = 1.0,
            accountUrl = "https://www.starbucks.com/account/rewards",
            awardSearchUrl = "https://www.starbucks.com/rewards",
            brandKeywords = listOf("starbucks"),
            dataAsOf = AS_OF,
            notes = "1 star per \$1 paid with a card, 2 per \$1 via a preloaded Starbucks Card; redemptions from 25 stars. Value per star varies a lot by what you redeem for.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.TARGET_CIRCLE,
            qualifyingMetric = QualifyingMetric.QUALIFYING_DOLLARS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(null, "Circle earnings/rewards typically expire per offer; check the app for dates."),
            estValueCentsPerPoint = 100.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.target.com/circle",
            awardSearchUrl = "https://www.target.com/",
            brandKeywords = listOf("target circle", "target.com"),
            dataAsOf = AS_OF,
            balanceUnit = BalanceUnit.DOLLARS,
        ),
        ProgramProfile(
            program = LoyaltyProgram.WALMART_REWARDS,
            qualifyingMetric = QualifyingMetric.QUALIFYING_DOLLARS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(null, "Walmart Cash from the Capital One card/Walmart+ has no fixed expiry; offer-based rewards do."),
            estValueCentsPerPoint = 100.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.walmart.com/account",
            awardSearchUrl = "https://www.walmart.com/",
            brandKeywords = listOf("walmart"),
            dataAsOf = AS_OF,
            balanceUnit = BalanceUnit.DOLLARS,
        ),
        ProgramProfile(
            program = LoyaltyProgram.COSTCO_REWARDS,
            qualifyingMetric = QualifyingMetric.QUALIFYING_DOLLARS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(null, "The annual Executive 2% reward certificate must be used within the year it's issued; the Citi card's reward certificate expires Dec 31 of the year issued."),
            estValueCentsPerPoint = 100.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.costco.com/myaccount",
            awardSearchUrl = "https://www.costco.com/",
            brandKeywords = listOf("costco"),
            dataAsOf = AS_OF,
            balanceUnit = BalanceUnit.DOLLARS,
        ),
        ProgramProfile(
            program = LoyaltyProgram.UBER_REWARDS,
            qualifyingMetric = QualifyingMetric.QUALIFYING_DOLLARS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(null, "Uber Cash from a credit-card credit typically expires; purchased Uber Cash doesn't. Check each credit's terms."),
            estValueCentsPerPoint = 100.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.uber.com/",
            awardSearchUrl = "https://www.uber.com/",
            brandKeywords = listOf("uber"),
            dataAsOf = AS_OF,
            balanceUnit = BalanceUnit.DOLLARS,
        ),
        ProgramProfile(
            program = LoyaltyProgram.DOORDASH,
            qualifyingMetric = QualifyingMetric.QUALIFYING_DOLLARS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(null, "Monthly DashPass/credit-card credits expire at month end if unused."),
            estValueCentsPerPoint = 100.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.doordash.com/",
            awardSearchUrl = "https://www.doordash.com/",
            brandKeywords = listOf("doordash", "dashpass"),
            dataAsOf = AS_OF,
            balanceUnit = BalanceUnit.DOLLARS,
        ),
        ProgramProfile(
            program = LoyaltyProgram.SEPHORA_BEAUTY_INSIDER,
            qualifyingMetric = QualifyingMetric.QUALIFYING_DOLLARS,
            tiers = listOf(
                TierLevel("VIB", 350, perks = "Higher point multipliers, better birthday gift, seasonal savings events"),
                TierLevel("Rouge", 1_000, perks = "Free shipping, top multipliers, Rouge-only rewards"),
            ),
            expiration = ExpirationPolicy(12, "Points expire after 12 months without a purchase."),
            estValueCentsPerPoint = 2.0,
            basePointsPerDollar = 1.0,
            eliteBonusByTier = mapOf("VIB" to 0.25, "Rouge" to 0.50),
            accountUrl = "https://www.sephora.com/profile/BeautyInsider",
            awardSearchUrl = "https://www.sephora.com/beauty/rewards",
            brandKeywords = listOf("sephora"),
            dataAsOf = AS_OF,
            notes = "Tier thresholds are calendar-year spend in USD.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.ULTA_ULTAMATE,
            qualifyingMetric = QualifyingMetric.QUALIFYING_DOLLARS,
            tiers = listOf(
                TierLevel("Platinum", 500, perks = "1.25 points per \$1, points don't expire while Platinum"),
                TierLevel("Diamond", 1_200, perks = "1.5 points per \$1, \$25 beauty-services card"),
            ),
            expiration = ExpirationPolicy(12, "Base-level members' points expire after 12 months of inactivity; Platinum/Diamond points don't expire."),
            estValueCentsPerPoint = 5.0,
            basePointsPerDollar = 1.0,
            eliteBonusByTier = mapOf("Platinum" to 0.25, "Diamond" to 0.50),
            accountUrl = "https://www.ulta.com/ultamaterewards",
            awardSearchUrl = "https://www.ulta.com/ultamaterewards",
            brandKeywords = listOf("ulta"),
            dataAsOf = AS_OF,
            notes = "Redemption value scales up with larger redemptions (100 pts ≈ \$3, 2,000 pts ≈ \$125), so 5¢ is a mid-range estimate.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.CVS_EXTRACARE,
            qualifyingMetric = QualifyingMetric.QUALIFYING_DOLLARS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(null, "ExtraBucks Rewards carry a printed expiry, typically a few weeks."),
            estValueCentsPerPoint = 100.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.cvs.com/extracare/home",
            awardSearchUrl = "https://www.cvs.com/",
            brandKeywords = listOf("cvs", "extracare"),
            dataAsOf = AS_OF,
            balanceUnit = BalanceUnit.DOLLARS,
        ),
        ProgramProfile(
            program = LoyaltyProgram.WALGREENS_MYWALGREENS,
            qualifyingMetric = QualifyingMetric.QUALIFYING_DOLLARS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(12, "Walgreens Cash rewards expire after 12 months of account inactivity."),
            estValueCentsPerPoint = 100.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.walgreens.com/mywalgreens",
            awardSearchUrl = "https://www.walgreens.com/",
            brandKeywords = listOf("walgreens"),
            dataAsOf = AS_OF,
            balanceUnit = BalanceUnit.DOLLARS,
        ),
        ProgramProfile(
            program = LoyaltyProgram.KROGER_PLUS,
            qualifyingMetric = QualifyingMetric.QUALIFYING_DOLLARS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(null, "Fuel points expire at the end of the month after they're earned."),
            estValueCentsPerPoint = 3.5,
            basePointsPerDollar = 1.0,
            accountUrl = "https://www.kroger.com/account/update",
            awardSearchUrl = "https://www.kroger.com/",
            brandKeywords = listOf("kroger", "ralphs", "fred meyer", "king soopers", "fry's", "smith's"),
            dataAsOf = AS_OF,
            notes = "Value assumes 100 fuel points = 10¢/gal on a 35-gallon fill; yours depends on tank size.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.BEST_BUY,
            qualifyingMetric = QualifyingMetric.QUALIFYING_DOLLARS,
            tiers = listOf(
                TierLevel("My Best Buy Plus", null, perks = "Paid tier: free 2-day shipping, exclusive prices, 60-day returns"),
                TierLevel("My Best Buy Total", null, perks = "Paid tier: adds protection plans and 24/7 tech support"),
            ),
            expiration = ExpirationPolicy(null, "Reward certificates expire ~60 days after issue; points from the Best Buy card expire after 12 months of inactivity."),
            estValueCentsPerPoint = 2.0,
            basePointsPerDollar = 5.0,
            accountUrl = "https://www.bestbuy.com/site/customer/myrewards",
            awardSearchUrl = "https://www.bestbuy.com/",
            brandKeywords = listOf("best buy", "bestbuy"),
            dataAsOf = AS_OF,
            notes = "Points only accrue with the My Best Buy credit card (5% back as points); tiers are paid memberships, not earned.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.CHIPOTLE_REWARDS,
            qualifyingMetric = QualifyingMetric.STATUS_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(6, "Points expire after 6 months of inactivity."),
            estValueCentsPerPoint = 0.6,
            basePointsPerDollar = 10.0,
            accountUrl = "https://www.chipotle.com/rewards",
            awardSearchUrl = "https://www.chipotle.com/rewards",
            brandKeywords = listOf("chipotle"),
            dataAsOf = AS_OF,
            notes = "10 points per \$1; an entrée costs about 1,625 points.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.DUNKIN_REWARDS,
            qualifyingMetric = QualifyingMetric.STATUS_POINTS,
            tiers = listOf(TierLevel("Boosted Status", null, perks = "12 points per \$1 for 3 months after 12 visits in a month")),
            expiration = ExpirationPolicy(6, "Points expire after 6 months of inactivity."),
            estValueCentsPerPoint = 0.6,
            basePointsPerDollar = 10.0,
            accountUrl = "https://www.dunkindonuts.com/en/dd-perks",
            awardSearchUrl = "https://www.dunkindonuts.com/en/dd-perks",
            brandKeywords = listOf("dunkin"),
            dataAsOf = AS_OF,
        ),
        ProgramProfile(
            program = LoyaltyProgram.PANERA_MYPANERA,
            qualifyingMetric = QualifyingMetric.STATUS_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(null, "Rewards are surprise/visit-based and each carries its own expiry (usually 60 days)."),
            estValueCentsPerPoint = 100.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.panerabread.com/en-us/mypanera.html",
            awardSearchUrl = "https://www.panerabread.com/",
            brandKeywords = listOf("panera"),
            dataAsOf = AS_OF,
            balanceUnit = BalanceUnit.DOLLARS,
            notes = "No points balance to speak of - the app tracks the dollar value of unredeemed rewards, if a statement states one.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.RAKUTEN,
            qualifyingMetric = QualifyingMetric.QUALIFYING_DOLLARS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(null, "Cash back is paid out quarterly ('Big Fat Check'/PayPal) and doesn't expire."),
            estValueCentsPerPoint = 100.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.rakuten.com/account",
            awardSearchUrl = "https://www.rakuten.com/",
            brandKeywords = listOf("rakuten"),
            dataAsOf = AS_OF,
            balanceUnit = BalanceUnit.DOLLARS,
            notes = "Shopping-portal cash back stacks on top of card rewards - worth checking before any online purchase. Members can opt to receive Amex MR instead of cash.",
        ),

        // ---------------------------------------------------------------
        // Outside the United States. These entries deliberately carry no
        // tier ladder and no point valuation: the ladders were revised
        // recently in several of these programmes and a US-cent valuation
        // needs an exchange rate the app does not hold, so both are left
        // unknown rather than guessed. Expiry rules are filled in only
        // where they could be checked against the programme's own terms;
        // the rest say so out loud.
        // ---------------------------------------------------------------
        ProgramProfile(
            program = LoyaltyProgram.QANTAS_FREQUENT_FLYER,
            qualifyingMetric = QualifyingMetric.STATUS_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(18, "Points expire after 18 months with no earning or redeeming activity; each eligible activity restarts the 18 months. Family Transfers and Qantas Business Rewards transfers do not count as activity."),
            estValueCentsPerPoint = 0.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.qantas.com/au/en/frequent-flyer.html",
            awardSearchUrl = "https://www.qantas.com/au/en/frequent-flyer.html",
            brandKeywords = listOf("qantas", "qantaslink", "jetstar"),
            dataAsOf = AS_OF,
            valuationIsKnown = false,
            notes = "Status is earned in Status Credits. The app holds no tier thresholds or point valuation for Qantas, so it tracks the balance and the expiry clock only.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.VELOCITY_FREQUENT_FLYER,
            qualifyingMetric = QualifyingMetric.STATUS_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(24, "Points expire 24 months after the last earn, redemption or transfer into the account. Family Pooling and family member transfers do not reset the clock."),
            estValueCentsPerPoint = 0.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.velocityfrequentflyer.com/",
            awardSearchUrl = "https://www.velocityfrequentflyer.com/",
            brandKeywords = listOf("velocity", "virgin australia"),
            dataAsOf = AS_OF,
            valuationIsKnown = false,
            notes = "Status is earned in Status Credits. No tier thresholds or point valuation are held for Velocity, so the app tracks the balance and the expiry clock only.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.EVERYDAY_REWARDS,
            qualifyingMetric = QualifyingMetric.QUALIFYING_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(null, "Woolworths removes points after a period of inactivity, but the app could not confirm the exact window, so it shows no countdown. Check the Everyday Rewards terms."),
            estValueCentsPerPoint = 0.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.everydayrewards.com.au/",
            awardSearchUrl = "https://www.everydayrewards.com.au/",
            brandKeywords = listOf("everyday rewards", "woolworths", "big w", "bws"),
            dataAsOf = AS_OF,
            valuationIsKnown = false,
            notes = "Points convert to in-store credit in Australian dollars. The app holds no conversion rate, so it shows the balance without a value.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.FLYBUYS,
            qualifyingMetric = QualifyingMetric.QUALIFYING_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(12, "Points expire unless you collect or redeem at least once every 12 months; the same 12 months of activity keeps the account itself open."),
            estValueCentsPerPoint = 0.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.flybuys.com.au/",
            awardSearchUrl = "https://www.flybuys.com.au/",
            brandKeywords = listOf("flybuys", "coles", "liquorland", "first choice liquor", "kmart", "target australia"),
            dataAsOf = AS_OF,
            valuationIsKnown = false,
            notes = "Points convert to Coles dollars or transfer to Velocity. The app holds no conversion rate, so it shows the balance without a value.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.BA_EXECUTIVE_CLUB,
            qualifyingMetric = QualifyingMetric.STATUS_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(36, "Avios expire after 36 months with no collecting or spending in the account; any activity restarts the 36 months."),
            estValueCentsPerPoint = 0.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.britishairways.com/travel/execclub/",
            awardSearchUrl = "https://www.britishairways.com/travel/redeem/execclub/",
            brandKeywords = listOf("british airways", "executive club", "avios", "iberia", "aer lingus", "vueling"),
            dataAsOf = AS_OF,
            valuationIsKnown = false,
            notes = "Avios sit in one balance shared with Iberia, Aer Lingus and Vueling. The tier ladder was reworked recently, so the app holds no thresholds and no valuation - balance and expiry clock only.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.VIRGIN_ATLANTIC_FLYING_CLUB,
            qualifyingMetric = QualifyingMetric.TILES,
            tiers = emptyList(),
            expiration = ExpirationPolicy(null, "Virgin Points no longer expire through inactivity - the old 36-month rule was dropped in 2020. They are still lost if the account is closed."),
            estValueCentsPerPoint = 0.0,
            basePointsPerDollar = null,
            accountUrl = "https://flywith.virginatlantic.com/",
            awardSearchUrl = "https://flywith.virginatlantic.com/",
            brandKeywords = listOf("virgin atlantic", "flying club", "virgin points"),
            dataAsOf = AS_OF,
            valuationIsKnown = false,
            notes = "Status is earned in Tiles. The app holds no tier thresholds or point valuation, so it tracks the balance only.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.TESCO_CLUBCARD,
            qualifyingMetric = QualifyingMetric.QUALIFYING_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(null, "The points balance has no inactivity clock, but points are converted into vouchers and a voucher expires 24 months after it is issued - so the deadline sits on each voucher, not on the balance."),
            estValueCentsPerPoint = 0.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.tesco.com/clubcard/",
            awardSearchUrl = "https://www.tesco.com/clubcard/",
            brandKeywords = listOf("tesco", "clubcard"),
            dataAsOf = AS_OF,
            valuationIsKnown = false,
            notes = "Vouchers are worth more through Reward Partners than in store, and the rate varies by partner, so the app holds no single valuation.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.NECTAR,
            qualifyingMetric = QualifyingMetric.QUALIFYING_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(12, "Points have no fixed expiry date, but an account with no earning or spending for 12 continuous months can be closed and its points lost. Scanning the card once a year is enough."),
            estValueCentsPerPoint = 0.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.nectar.com/",
            awardSearchUrl = "https://www.nectar.com/",
            brandKeywords = listOf("nectar", "sainsbury", "argos"),
            dataAsOf = AS_OF,
            valuationIsKnown = false,
            notes = "Points spend in pounds at Sainsbury's, Argos and eBay. The app holds no conversion rate, so it shows the balance without a value.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.BOOTS_ADVANTAGE,
            qualifyingMetric = QualifyingMetric.QUALIFYING_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(12, "Points expire unless the card is used for a Boots purchase within a year; an account left inactive for four years is closed altogether."),
            estValueCentsPerPoint = 0.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.boots.com/shopping/advantage-card",
            awardSearchUrl = "https://www.boots.com/shopping/advantage-card",
            brandKeywords = listOf("boots", "advantage card"),
            dataAsOf = AS_OF,
            valuationIsKnown = false,
            notes = "Points spend in pounds in store. The app holds no conversion rate, so it shows the balance without a value.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.AIR_CANADA_AEROPLAN,
            qualifyingMetric = QualifyingMetric.QUALIFYING_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(18, "Expiry after 18 months of inactivity is suspended until 29 November 2026, when it resumes. There is no grace period on that date: an account with no qualifying activity in the preceding 18 months loses its points immediately."),
            estValueCentsPerPoint = 0.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.aircanada.com/ca/en/aco/home/aeroplan.html",
            awardSearchUrl = "https://www.aircanada.com/ca/en/aco/home/aeroplan.html",
            brandKeywords = listOf("air canada", "aeroplan", "air canada rouge"),
            dataAsOf = AS_OF,
            valuationIsKnown = false,
            notes = "Status is earned in Status Qualifying Miles or Segments plus Status Qualifying Dollars. The app holds no thresholds and no valuation, so it tracks the balance and the resuming expiry clock only.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.WESTJET_REWARDS,
            qualifyingMetric = QualifyingMetric.QUALIFYING_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(null, "The programme is mid-transition from WestJet dollars to points and the app could not confirm which expiry rule now applies, so it shows no countdown. WestJet may deactivate an account left inactive for 24 months."),
            estValueCentsPerPoint = 0.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.westjet.com/en-ca/rewards",
            awardSearchUrl = "https://www.westjet.com/en-ca/rewards",
            brandKeywords = listOf("westjet", "swoop"),
            dataAsOf = AS_OF,
            valuationIsKnown = false,
            notes = "Balances are moving from WestJet dollars to a points currency. Record whichever your latest statement shows, and re-check the rules before planning around them.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.PC_OPTIMUM,
            qualifyingMetric = QualifyingMetric.QUALIFYING_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(12, "Points expire after 12 months with no earning or redeeming; scanning the card or using a PC Financial card restarts the clock."),
            estValueCentsPerPoint = 0.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.pcoptimum.ca/",
            awardSearchUrl = "https://www.pcoptimum.ca/",
            brandKeywords = listOf("pc optimum", "loblaws", "shoppers drug mart", "no frills", "real canadian superstore", "zehrs", "fortinos", "provigo"),
            dataAsOf = AS_OF,
            valuationIsKnown = false,
            notes = "Points redeem for Canadian dollars off a grocery or pharmacy bill. The app holds no conversion rate, so it shows the balance without a value.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.SCENE_PLUS,
            qualifyingMetric = QualifyingMetric.QUALIFYING_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(24, "Points expire after 24 months of inactivity. Members holding a Scene+ credit card are exempt while that card is open."),
            estValueCentsPerPoint = 0.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.sceneplus.ca/",
            awardSearchUrl = "https://www.sceneplus.ca/",
            brandKeywords = listOf("scene+", "scene plus", "cineplex", "sobeys", "safeway canada", "foodland", "iga", "empire theatres"),
            dataAsOf = AS_OF,
            valuationIsKnown = false,
            notes = "Redemption value differs by category (groceries, travel, cinema), so the app holds no single valuation. If you hold a Scene+ credit card, the expiry clock does not apply to you.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.AIR_MILES_CA,
            qualifyingMetric = QualifyingMetric.QUALIFYING_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(24, "Miles have no expiry date of their own, but an account with no earning, redeeming or transferring for 24 consecutive months expires."),
            estValueCentsPerPoint = 0.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.airmiles.ca/",
            awardSearchUrl = "https://www.airmiles.ca/",
            brandKeywords = listOf("air miles", "airmiles"),
            dataAsOf = AS_OF,
            valuationIsKnown = false,
            notes = "Cash Miles and Dream Miles redeem very differently, so the app holds no single valuation. Record the balance you care about and check the programme for its rate.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.SAA_VOYAGER,
            qualifyingMetric = QualifyingMetric.QUALIFYING_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(null, "Voyager uses a calendar-anchored rule rather than a rolling clock - miles lapse on 31 March, with activity during the year protecting them - and the app could not pin the exact terms down, so it shows no countdown. Check Voyager's own terms before relying on a date."),
            estValueCentsPerPoint = 0.0,
            basePointsPerDollar = null,
            accountUrl = "https://voyager.flysaa.com/",
            awardSearchUrl = "https://voyager.flysaa.com/",
            brandKeywords = listOf("south african airways", "flysaa", "voyager"),
            dataAsOf = AS_OF,
            valuationIsKnown = false,
            notes = "Status is earned in Tier Miles. The app holds no tier thresholds or valuation, so it tracks the balance only.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.FNB_EBUCKS,
            qualifyingMetric = QualifyingMetric.QUALIFYING_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(null, "eBucks do not expire, so a balance can be built up over time."),
            estValueCentsPerPoint = 0.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.ebucks.com/",
            awardSearchUrl = "https://www.ebucks.com/",
            brandKeywords = listOf("ebucks", "fnb", "first national bank", "rmb private bank"),
            dataAsOf = AS_OF,
            valuationIsKnown = false,
            notes = "eBucks are denominated in rand, not US cents, so the app shows the balance without converting it. Your earn rate depends on your eBucks level, which the app does not track.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.CLICKS_CLUBCARD,
            qualifyingMetric = QualifyingMetric.QUALIFYING_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(null, "Clicks issues cash-back vouchers on a schedule and those carry their own deadlines. The app could not verify them, so it shows no countdown - check your ClubCard statement."),
            estValueCentsPerPoint = 0.0,
            basePointsPerDollar = null,
            accountUrl = "https://clicks.co.za/clubcard",
            awardSearchUrl = "https://clicks.co.za/clubcard",
            brandKeywords = listOf("clicks", "clubcard"),
            dataAsOf = AS_OF,
            valuationIsKnown = false,
            notes = "Rewards arrive as rand cash-back vouchers rather than a spendable point balance, so the app shows whatever figure your statement states, without valuing it.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.PICK_N_PAY_SMART_SHOPPER,
            qualifyingMetric = QualifyingMetric.QUALIFYING_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(null, "The app could not verify Smart Shopper's expiry terms, so it shows no countdown. Check Pick n Pay's own terms."),
            estValueCentsPerPoint = 0.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.pnp.co.za/smartshopper",
            awardSearchUrl = "https://www.pnp.co.za/smartshopper",
            brandKeywords = listOf("pick n pay", "smart shopper", "smartshopper"),
            dataAsOf = AS_OF,
            valuationIsKnown = false,
            notes = "Points convert to rand off a shop, and can be linked to an FNB eBucks account. The app holds no conversion rate, so it shows the balance without a value.",
        ),
        ProgramProfile(
            program = LoyaltyProgram.DISCOVERY_VITALITY,
            qualifyingMetric = QualifyingMetric.QUALIFYING_POINTS,
            tiers = emptyList(),
            expiration = ExpirationPolicy(null, "Vitality status runs on a membership year and Discovery Miles carry their own terms. The app has verified neither, so it shows no countdown."),
            estValueCentsPerPoint = 0.0,
            basePointsPerDollar = null,
            accountUrl = "https://www.discovery.co.za/portal/individual/vitality",
            awardSearchUrl = "https://www.discovery.co.za/portal/individual/vitality",
            brandKeywords = listOf("discovery vitality", "vitality", "discovery miles", "vitality travel"),
            dataAsOf = AS_OF,
            valuationIsKnown = false,
            notes = "Vitality status (Blue through Diamond) drives discounts rather than a points balance; Discovery Miles are the spendable currency. Record whichever your statement reports.",
        ),
    ).associateBy { it.program }

    fun profileFor(program: LoyaltyProgram): ProgramProfile =
        profiles[program] ?: unknownProfile(program)

    /**
     * Stand-in for a programme the catalog does not cover, so one found in email
     * can be tracked without inventing facts about it. Every field that would
     * otherwise be a guess is empty: no tier ladder, no expiry rule, no earning
     * rate, and a valuation explicitly marked unknown so balances are never
     * priced. Previously this case threw, which meant such programmes could not
     * exist at all.
     */
    private fun unknownProfile(program: LoyaltyProgram): ProgramProfile = ProgramProfile(
        program = program,
        qualifyingMetric = QualifyingMetric.QUALIFYING_POINTS,
        tiers = emptyList(),
        expiration = ExpirationPolicy(inactivityMonths = null, summary = "Expiry rules for this programme are not in the app - check the programme's own terms."),
        estValueCentsPerPoint = 0.0,
        basePointsPerDollar = null,
        accountUrl = "",
        awardSearchUrl = "",
        brandKeywords = emptyList(),
        dataAsOf = "not in the built-in catalog",
        notes = "Found in your email. The app has no tier, expiry or valuation data for it, so it shows the balance only.",
        valuationIsKnown = false,
    )

    /** Program whose brands match a free-text provider/hotel name, if any. Travel programs only by default - a booking never "credits" to a shop. */
    fun programForProvider(providerText: String?, includeShops: Boolean = false): LoyaltyProgram? {
        if (providerText.isNullOrBlank()) return null
        val haystack = providerText.lowercase()
        return profiles.values
            .filter { includeShops || it.program.kind != LoyaltyProgramKind.SHOP }
            .filter { profile -> profile.brandKeywords.any { haystack.contains(it) } }
            // Longest keyword wins so "Hyatt Place" beats a stray "place" style false positive elsewhere.
            .maxByOrNull { profile -> profile.brandKeywords.filter { haystack.contains(it) }.maxOf { it.length } }
            ?.program
    }
}
