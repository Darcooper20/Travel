...
/**
 * Scans Gmail (read-only) for emails from known hotel and airline loyalty programs and
...
class GmailScanRepository @Inject constructor(
    ...
    suspend fun scanForLoyaltyAccounts(onProgress: suspend (String) -> Unit = {}): Result<List<LoyaltyAccount>> {
        ...
        return try {
            ...
            for (item in extracted) {
                val program = runCatching { LoyaltyProgram.valueOf(item.program) }.getOrNull() ?: continue
    ...
    private data class EmailCandidate(val program: LoyaltyProgram, val subject: String, val body: String)
    ...
    companion object {
        ...
        private val EXTRACTION_SYSTEM_PROMPT: String
            get() {
                ...
                return """
                    You extract hotel and airline loyalty program membership
                    details from emails. You will be given several emails,
                    each labeled "Email N" with a program hint, subject, and
                    body text (the body may be messy, HTML-stripped text).
                    For EACH email that actually contains a membership/loyalty
                    number, elite tier/status, or points/miles balance, output
                    one JSON object for it. Skip emails that are just
                    marketing/promotional with no account-specific details -
                    do not guess or invent numbers that are not actually
                    present in the text. Respond with ONLY a JSON array (no
                    markdown code fences, no other text), where each item
                    matches exactly this shape:
                    {
                      "program": one of $programNames,
                      "membershipNumber": string or null,
                      "tier": string or null (e.g. "Gold", "Platinum", "Diamond", "Medallion Silver", "Premier 1K"),
                      "pointsBalance": string or null (as written, e.g. "42,500 points" or "18,200 miles"),
