...
| Finding newly-catalogued programmes (b251510, 991f77c) | **all green** at 991f77c: 157 unit tests, 16 instrumented tests on both emulators. Two reasons a programme added by an app update was never picked up: the incremental search window excluded all of its mail, and the per-sync email cap truncated a list it always sat at the end of. `b251510` failed `build` on a type-inference error in the new test itself (`roundRobin(emptyList())` infers nothing) while both emulator jobs passed |
| Membership numbers (3fdd88d … 02e390a) | **all green** at 02e390a: 165 unit tests, 16 instrumented tests on both emulators, debug and release (R8) APKs. Notable not for a test failure but for a CI one: `b051f69` built and tested clean, then failed only at the publish step with "Error saving asset". Three pushes in fifteen minutes had raced to replace the single APK on the `debug-latest` release, deleting the old asset and failing to save the new one, so the README's download link 404'd while the run looked like it had failed cosmetically at the last step. The workflow now cancels superseded runs on the same branch (`concurrency` + `cancel-in-progress`). Lesson recorded because it is invisible from test results alone: a green build is not a published build, and the release has to be checked separately |
